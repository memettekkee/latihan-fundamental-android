...
class MyPreferenceFragment : PreferenceFragmentCompat(),
    ...
    override fun onSharedPreferenceChanged(sP: SharedPreferences, key: String?) {
        if (key == NAME) {
            namePreference.summary = sP.getString(NAME, DEFAULT_VALUE)
        }

        if (key == EMAIL) {
            emailPreference.summary = sP.getString(EMAIL, DEFAULT_VALUE)
        }
        if (key == AGE) {
            agePreference.summary = sP.getString(AGE, DEFAULT_VALUE)
        }
        if (key == PHONE) {
            phonePreference.summary = sP.getString(PHONE, DEFAULT_VALUE)
        }
        if (key == LOVE) {
            isLoveMuPreference.isChecked = sP.getBoolean(LOVE, false)
        }
    }
